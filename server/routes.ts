...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/batteries", async (req: Request, res: Response) => {
    try {
      ...
      // Handle initialDate - ensure it's a valid date string
      let initialDateStr;
      try {
        ...
        console.error("Error parsing initialDate:", err);
        initialDateStr = now;
      }
  ...
  (global as any).broadcastBatteryUpdate = (battery: any, history: any) => {
    broadcastUpdate('battery_update', { battery, history });
  };

  (global as any).broadcastRecommendation = (recommendation: any) => {
    broadcastUpdate('recommendation', { recommendation });
  };
